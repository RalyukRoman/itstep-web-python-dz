    ...
    if tags:
        try:
            ...
            if not isinstance(tag_list, list):
                raise HTTPException(status_code=400, detail="Tags must be a valid JSON list of strings")
        except json.JSONDecodeError:
            raise HTTPException(status_code=400, detail="Invalid JSON format for tags")

    return await service.create_post(content, current_user.id, image, tag_list)
