...
class PostService:
    ...
    async def create_post(
        ...
        tags_entities = []
        if tag_names:
            tag_repo = TagRepository(self.repository.db)
            for name in tag_names:
                cleaned_name = name.strip().lower()
                if not cleaned_name:
                    continue
                tag_data = await tag_repo.get_by_name(cleaned_name)
                if tag_data:
                    _, tag_entity = tag_data
                else:
                    _, tag_entity = await tag_repo.create_tag(cleaned_name)
                tags_entities.append(tag_entity)

        _, post = await self.repository.create_post(post_data, tags_entities=tags_entities)
        return post
    ...
    async def get_post_detail(
        ...
        post_data = await self.repository.get_post_by_id(post_id)
        if not post_data:
            return None
